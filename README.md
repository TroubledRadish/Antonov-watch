...
## IMPORTANT: put these files in the repo ROOT

`watch.py`, `state.json`, and the `.github/` folder must sit at the **top
level** of the repo — not inside a subfolder. GitHub only runs workflows
found at `.github/workflows/` in the repo root, and the workflow calls
`python3 watch.py` from the root. If you uploaded a folder by mistake,
recreate the files at the root with **Add file -> Create new file** and type
the path (e.g. `.github/workflows/watch.yml`) into the filename box.
...
3. Add these files at the repo **root** (see the box above).
4. In the repo: **Settings -> Secrets and variables -> Actions -> New
   repository secret**.
   - Name: `NTFY_TOPIC`
   - Value: your topic string from step 1 — **just the topic**, no
     `https://`, no `ntfy.sh/`, no slash.
...
Data (c) [adsb.fi](https://adsb.fi), used for personal, non-commercial purposes.
